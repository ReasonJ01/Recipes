...
- Images are optional. A recipe without a photo is ready to publish. Add an image only when supplied or requested, in `public/images/`. For generated recipe images, read `IMAGE_PROMPT.md` and use its house style unless the user asks for something different.
